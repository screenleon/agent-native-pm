...
// T-S5b-6: DELETE /api/me/account-bindings/:id scrubs stale cli_health entries
// from all local_connectors belonging to the same user (R12 / D3, Path B S5b).
func TestS5b_6DeleteBindingScrubsCliHealth(t *testing.T) {
	db := testutil.OpenTestDB(t)
	if _, err := db.Exec(`
		INSERT INTO users (id, username, email, password_hash, role, is_active)
		VALUES ('local-admin', 'local', 'local@example.com', '', 'admin', TRUE)
	...
	bs := store.NewAccountBindingStore(db, nil)
	dialect := database.NewDialect("")
	lcs := store.NewLocalConnectorStore(db, dialect)

	h := handlers.NewAccountBindingHandler(bs).
		WithLocalMode(true).
		WithLocalConnectorStore(lcs)
	...
	// Create a CLI binding.
	binding, err := bs.Create("local-admin", models.CreateAccountBindingRequest{
		ProviderID: models.AccountBindingProviderCLIClaude,
		Label:      "test-cli",
		ModelID:    "claude-sonnet-4-6",
	})
	...
	// Create a connector row (simulating a paired connector).
	if _, err := db.Exec(`
		INSERT INTO local_connectors (id, user_id, label, platform, client_version, status, capabilities, token_hash, last_error, created_at, updated_at)
		VALUES ('conn-1', 'local-admin', 'My Machine', 'linux', '1.0', 'online', '{}', 'hash1', '', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
	`); err != nil {
		t.Fatalf("insert connector: %v", err)
	}

	// Inject cli_health metadata for the binding we're about to delete.
	meta := `{"cli_health":{"` + binding.ID + `":{"status":"healthy","version_string":"claude 1.5","checked_at":"2024-01-01T00:00:00Z","probe_error_message":""}}}`
	if _, err := db.Exec(`UPDATE local_connectors SET metadata = $1 WHERE id = 'conn-1'`, meta); err != nil {
		t.Fatalf("inject metadata: %v", err)
	}

	// Sanity-check: health key is present before delete.
	var raw []byte
	if err := db.QueryRow(`SELECT metadata FROM local_connectors WHERE id = 'conn-1'`).Scan(&raw); err != nil {
		t.Fatalf("read metadata before: %v", err)
	}
	if !strings.Contains(string(raw), binding.ID) {
		t.Fatalf("expected binding ID in metadata before delete: %s", string(raw))
	}

	// DELETE the binding via the handler.
	...
	// After delete, the cli_health key for this binding should be gone.
	if err := db.QueryRow(`SELECT metadata FROM local_connectors WHERE id = 'conn-1'`).Scan(&raw); err != nil {
		t.Fatalf("read metadata after: %v", err)
	}
	if strings.Contains(string(raw), binding.ID) {
		t.Fatalf("expected binding ID to be scrubbed from metadata, but still present: %s", string(raw))
	}
}
