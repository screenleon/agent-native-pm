...
func NewSearchStore(db *sql.DB, dialect database.Dialect) *SearchStore {
...
// On PostgreSQL the search uses tsvector/tsquery; on SQLite it falls back to LIKE.
...
func (s *SearchStore) searchTasks(query, projectID, taskStatus string) (*sql.Rows, error) {
	if s.dialect.IsSQLite() {
		return s.searchTasksSQLite(query, projectID, taskStatus)
	}
...
func (s *SearchStore) searchTasksSQLite(query, projectID, taskStatus string) (*sql.Rows, error) {
	like := "%" + query + "%"
...
func (s *SearchStore) searchDocuments(query, projectID, docType string, staleOnly *bool) (*sql.Rows, error) {
	if s.dialect.IsSQLite() {
		return s.searchDocumentsSQLite(query, projectID, docType, staleOnly)
	}
	args := []interface{}{query}
...
func (s *SearchStore) searchDocumentsSQLite(query, projectID, docType string, staleOnly *bool) (*sql.Rows, error) {
	like := "%" + query + "%"
	args := []interface{}{like, like}
	where := "(d.title LIKE $1 OR d.description LIKE $2)"
